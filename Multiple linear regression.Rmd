...
model.diag.metrics <- augment(m_bty_gen)

ggplot(model.diag.metrics, aes(bty_avg, score)) +
  geom_point() +
  stat_smooth(method = lm, se = FALSE) +
  geom_segment(aes(xend = bty_avg, yend = .fitted), color = "red", size = 0.3)
```
...
```{r p-val-select}
# type your code for the Exercise here, and Knit
reduced_model <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
...
```{r adj-rsq-select}
# type your code for the Exercise here, and Knit
model_1 <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_profs + cls_credits 
             + pic_outfit + pic_color, data = evals)
summary(model_1)$adj.r.squared

model_2 <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
summary(model_2)$adj.r.squared

model_3 <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_level + cls_profs + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
summary(model_3)$adj.r.squared

model_4 <- lm(score ~ ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_profs + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
