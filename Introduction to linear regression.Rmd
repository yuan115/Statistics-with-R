...
model_Q4 <- lm(runs ~ homeruns, data = mlb11)
summary(model_Q4)
```
...
(projection <- coefficients(m1)[2]  * 5579 + coefficients(m1)[1])

(residual <- 713 - projection)
```
...
ggplot(data = mlb11, aes(hits, runs)) +
  geom_point() +
  stat_smooth(method = "lm", se = FALSE)
```
...
lm_Q9 <- lm(runs ~ at_bats + hits + wins + bat_avg, data = mlb11)
...
(obs_R <- summary(lm(runs ~ new_obs, data = mlb11))$r.squared)
(slug_R <- summary(lm(runs ~ new_slug, data = mlb11))$r.squared)
(onbase_R <- summary(lm(runs ~ new_onbase, data = mlb11))$r.squared)
```
...
```{r diag}
# type your code for the Exercise here, and Knit
list_R <- list(R = c(obs_R, slug_R, onbase_R))

plot(list_R$R, main = "Different R-squared in three models",
     xlab = "models", ylab = "R-squared", xaxt='n')
axis(1, at = c(1, 2, 3))
```
