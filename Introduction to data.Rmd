...
nycflights %>%
  group_by(month) %>%
  summarise(mean_time = mean(dep_time)) %>%
...
library(sqldf)
Info <- sqldf("SELECT dep_type
              ,arr_type
       FROM nycflights
       WHERE dep_type = 'delayed'
       AND arr_type == 'on time'")



nrow(Info) / nrow(sqldf("SELECT dep_type, arr_type FROM nycflights WHERE dep_type = 'delayed'"))
