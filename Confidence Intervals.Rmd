...
# type your code for the Exercise here, and Knit
samp %>%
  summarise(result = ifelse(params > mean(area) - z_star_95 * (sd(area) / sqrt(n)) & 
              params < mean(area) + z_star_95 * (sd(area) / sqrt(n)), "yes", "no"))
```
...
ggplot(ci_99_data, aes(ci_bounds, ci_id, group = ci_id, color = capture_mu)) +
  geom_point(size = 2) +
  geom_line() +           
