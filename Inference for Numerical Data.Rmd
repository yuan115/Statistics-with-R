...
inference(y = weeks, data = nc, statistic = "mean", type = "ci", method = "theoretical", conf_level = .9)
```
...
```{r gained-mature-ht}
# type your code for the Exercise here, and Knit
inference(y = weeks, x = habit, data = nc, statistic = "mean", type = "ci", method = "theoretical", conf_level = 0.95)
```
...
mean <- mean(nc$mage, na.rm = TRUE)

nc_edited <- nc %>% 
  mutate(label = 0) %>% 
  na.omit()

for (i in 1:nrow(nc_edited)) {
  if ((nc_edited$mage[i] >= mean) == TRUE) {
    nc_edited$label[i] = 1
  }
}

inference(y = weeks, x = label, data = nc_edited, statistic = "mean", type = "ci", method = "theoretical", conf_level = 0.95)
```
